...
| ID  | Task                                                                              | Acceptance                                           |
| --- | --------------------------------------------------------------------------------- | ---------------------------------------------------- |
| A-1 | `lib/auth/password.ts` — bcrypt 또는 argon2로 해시·비교                           | 단위 테스트: 정상 비교 / 오타 / 빈 문자열 3 케이스   |
| A-2 | `lib/auth/session.ts` — JWT(HS256) sign/verify, 만료 24h                          | 만료 토큰 거부, payload `{ sub: "admin", iat, exp }` |
| A-3 | `ADMIN_PASSWORD_HASH`, `SESSION_SECRET` 환경 변수 추가, `.env.local.example` 갱신 | 두 값 없을 때 부팅 시 명확한 에러                    |
| A-4 | `app/admin-login/page.tsx` — 비밀번호 입력 폼, Server Action으로 검증             | 정상 시 세션 쿠키 set + `/admin` 리다이렉트          |
| A-5 | `middleware.ts` — `/admin/*` 보호, 세션 없으면 `/admin-login` 리다이렉트          | 미인증 접근 시 302 + Location 헤더                   |
| A-6 | 비밀번호 실패 시 1초 sleep + (Phase D의) rate limit 적용                          | 무차별 대입 마찰                                     |

**Done**

- 단위 테스트 (`lib/auth/password.test.ts`, `lib/auth/session.test.ts`) 통과
- V9, V10 Playwright 시나리오 통과
- 세션 쿠키 속성 검증 (A8)

**리스크**

- JWT secret 유출 → `.env.local`만 사용, 빌드 산출물 grep을 출시 게이트에 추가
- bcrypt가 Edge runtime에서 미동작 → middleware는 `nodejs` runtime 강제
